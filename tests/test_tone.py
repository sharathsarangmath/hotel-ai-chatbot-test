import pytest
from src.chatbot import ask_concierge
import json

def load_cases(category=None):
    with open("data/tone_test_data.json") as f:
        cases = json.load(f)
    if category:
        cases = [c for c in cases if c["category"]==category]
    return cases



class TestTone:
    """Tests that verify the chatbot always maintains a polite and professional tone."""

    @pytest.mark.parametrize("case",load_cases())
    def test_tone(self, case):
        """Chatbot should always use polite language in responses."""
        response = ask_concierge(case["input"])

        if case ["category"]=="unprofessional":
            assert not any(
                phrase in response.lower()
                for phrase in case["forbidden_keywords"]
            ),(
                f"Test ID: {case['id']}\n"
                f"Category: {case['category']}\n"
                f"Input: {case['input']}\n"
                f"Forbidden phrase found in: {response}"
            )
        else:
            assert any(
                keyword in response.lower()
                for keyword in case["expected_keywords"]
            ), (
                f"Test ID: {case['id']}\n"
                f"Category: {case['category']}\n"
                f"Input: {case['input']}\n"
                f"Expected one of: {case['expected_keywords']}\n"
                f"Got: {response}"
            )
        